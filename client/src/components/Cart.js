...
function Cart({ user, setUser }) {
  ...
  if (user.cart.cart.length === 0) {
    return (
      <div>
        <h3 className="text-capitalize text-center" id="cart-h3">
          Hi {user.name}, You're cart is empty.
        </h3>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-75" id="margin-small">
      ...
      <h4 className="text-center cart-h4">
        Cart total: ${parseFloat(addZeros(user.cart.total)).toFixed(2)}
      </h4>
      <button className="btn btn-light btn-lg" onClick={onPurchase}>
        Purchase
      </button>
    </div>
  );
